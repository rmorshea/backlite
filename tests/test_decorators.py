...
def test_cached_function_with_expiration():
    cache = CleanCache("test.db")

    call_count = 0

    @cached(storage=cache, expiration=timedelta(seconds=0))
    def expensive_function(_: int) -> None:
        nonlocal call_count
        call_count += 1
...
async def test_async_cached_function_with_expiration():
    cache = CleanCache("test.db")

    call_count = 0

    @async_cached(storage=cache, expiration=timedelta(seconds=0))
    async def expensive_function(_: int) -> None:
        nonlocal call_count
        call_count += 1
